...
public class Carrito extends AppCompatActivity {
    ...
    private void ejecutarQuery() {
        ...
        if (c.moveToLast()) {
            modelo = obtenerDatos(c, modelo);
        } else {
            DbRegistros m = new DbRegistros();
            m.setNombre("No hay artículos en el carrito");
            modelo.add(m);
        }
    ...
    private ArrayList<DbRegistros> obtenerDatos(Cursor c, ArrayList<DbRegistros> modelo) {
        do {
    ...
    public void borrarPedido(View view) {
        try {
            ...
            mostrarMensaje("SE HA VACIADO EL CARRITO");
